                            ...
                            >
                                ...
                                <div className="space-y-4 relative">
                                    <div className="flex items-center gap-3 text-sm text-zinc-500 font-medium">
                                        ...
                                        <div>
                                            <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none">Login Username</p>
                                            <span className="font-bold text-black text-xs">{worker.email?.split('@')[0]}</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between text-sm text-zinc-500 font-medium pr-2">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-zinc-50 flex items-center justify-center group-hover:bg-black/5 transition-colors">
                                                <Key className="w-4 h-4 text-black" />
                                            </div>
                                            <div>
                                                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none">Password</p>
                                                <span className="font-bold text-black text-xs font-mono">
                                                    {showPasswords[worker.id] ? `${worker.email?.split('@')[0]}123` : '••••••••'}
                                                </span>
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => setShowPasswords(prev => ({ ...prev, [worker.id]: !prev[worker.id] }))}
                                            className="p-1.5 hover:bg-zinc-100 rounded-lg text-zinc-400 hover:text-black transition-colors"
                                            title={showPasswords[worker.id] ? "Hide Password" : "Show Password"}
                                        >
                                            {showPasswords[worker.id] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                        </button>
                                    </div>
                                    <div className="flex items-center gap-3 text-sm text-zinc-500 font-medium">
                                        <div className="w-8 h-8 rounded-lg bg-zinc-50 flex items-center justify-center group-hover:bg-black/5 transition-colors">
                                ...
                                <div className="mt-8 pt-8 border-t border-zinc-50 flex items-center justify-between gap-2">
                                    {worker.qr_code_url ? (
                                        <button
                                            type="button"
                                            onClick={() => setSelectedQrCode(worker.qr_code_url)}
                                            className="px-4 py-2.5 bg-zinc-50 hover:bg-black hover:text-white text-zinc-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 flex items-center gap-2 border border-zinc-100 shadow-sm active:scale-95"
                                        >
                                            <QrCode className="w-4 h-4" />
                                            <span>View QR Code</span>
                                        </button>
                                    ) : (
                                        <div className="text-[9px] font-bold text-zinc-300 uppercase tracking-widest">No QR Code</div>
                                    )}
                                    <button
                                ...
                                <div className="space-y-8 flex-1 w-full">
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        <div className="p-4 bg-zinc-50 rounded-2xl flex items-center gap-4 border border-zinc-100">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">Employee Login Username</p>
                                                <p className="font-black text-base text-black">{selectedWorker.email?.split('@')[0]}</p>
                                                <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Auth Email: {selectedWorker.email}</p>
                                            </div>
                                        </div>

                                        <div className="p-4 bg-zinc-50 rounded-2xl flex items-center justify-between border border-zinc-100">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-black text-white rounded-xl flex items-center justify-center shadow-sm">
                                                    <Key className="w-5 h-5" />
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">Account Password</p>
                                                    <p className="font-mono font-black text-sm text-black">
                                                        {showPasswords[selectedWorker.id] ? `${selectedWorker.email?.split('@')[0]}123` : '••••••••'}
                                                    </p>
                                                    <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Click eye icon to reveal</p>
                                                </div>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => setShowPasswords(prev => ({ ...prev, [selectedWorker.id]: !prev[selectedWorker.id] }))}
                                                className="p-2 hover:bg-zinc-200/50 rounded-xl text-zinc-400 hover:text-black transition-colors"
                                                title={showPasswords[selectedWorker.id] ? "Hide Password" : "Show Password"}
                                            >
                                                {showPasswords[selectedWorker.id] ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                            </button>
                                        </div>
                                        ...
                                        <div className="p-4 bg-zinc-50 rounded-2xl flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold text-sm text-black truncate">{selectedWorker.contact_email || selectedWorker.email}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-4 bg-zinc-50 rounded-2xl flex items-center justify-between gap-4">
                                            ...
                                            {selectedWorker.qr_code_url ? (
                                                <button
                                                    type="button"
                                                    onClick={() => setSelectedQrCode(selectedWorker.qr_code_url)}
                                                    className="px-3.5 py-2 bg-black text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-zinc-800 transition-all flex items-center gap-1.5 active:scale-95 shrink-0"
                                                >
